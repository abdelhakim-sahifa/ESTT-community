...
export default function ProjectSubmissionCard({
    ...
    return (
        <article className="overflow-hidden rounded-xl border-2 border-border bg-card shadow-none transition-all hover:border-orange-500/50 group">
            <Link href={detailUrl} className="block relative">
                ...
                {!image && <div className="relative h-28 sm:h-40 w-full bg-muted flex items-center justify-center text-muted-foreground"><Eye className="h-8 w-8" /></div>}
            </Link>
            ...
            <div className="space-y-3 sm:space-y-5 p-3 sm:p-5">
                <div>
                    ...
                    <Link href={detailUrl}>
                        <h3 className="text-sm sm:text-lg md:text-xl font-black leading-tight text-foreground line-clamp-1 hover:text-primary transition-colors">
                            {submission.title}
                        </h3>
                    </Link>
                    <p className="mt-1 sm:mt-2 line-clamp-2 text-[10px] sm:text-xs md:text-sm text-muted-foreground">{submission.description}</p>
                </div>

                <div className="flex items-center justify-between rounded-lg sm:rounded-xl border border-border bg-muted p-2 sm:p-4">
                    <div>
                        <p className="text-[9px] sm:text-xs uppercase tracking-wide text-muted-foreground">Votes</p>
                ...
                <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <p className="text-[8px] uppercase tracking-wide text-muted-foreground">Auteur</p>
                            <p className="text-[10px] font-semibold text-foreground">{submission.authorName}</p>
                        </div>
                    </div>

                    <Button variant="outline" size="sm" asChild className="rounded-full border-border h-8 text-[10px] font-bold uppercase transition-colors hover:bg-slate-900 hover:text-white">
