        ...
        <article className="overflow-hidden rounded-xl border-2 border-border bg-card shadow-none transition-all hover:border-orange-500/50">
            ...
            <div className="space-y-3 sm:space-y-5 p-3 sm:p-5">
                <div>
                    ...
                    <h3 className="text-sm sm:text-lg md:text-xl font-black leading-tight text-foreground line-clamp-1">{project.title}</h3>
                    <p className="mt-1 sm:mt-2 line-clamp-2 max-w-2xl text-[10px] sm:text-xs md:text-sm text-muted-foreground">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-2">
                    {project.tags.slice(0, 3).map((tag) => (
                        <span
                            key={tag}
                            className="rounded-full border border-border bg-card px-2 py-0.5 sm:px-3 sm:py-1 text-[8px] sm:text-[11px] font-semibold text-muted-foreground"
                        >
                            {tag}
                        </span>
                    ))}
                ...
                <div className="grid gap-1 sm:gap-3 grid-cols-3">
                    <div className="rounded-lg sm:rounded-xl border border-border bg-card p-1.5 sm:p-3 shadow-sm">
                        <div className="flex items-center gap-1 sm:gap-2 text-[7px] sm:text-xs font-bold uppercase tracking-wide text-muted-foreground">
                            <Layers3 className="h-2.5 w-2.5 sm:h-3.5 sm:w-3.5" />
                            <span className="truncate">Soumissions</span>
                        </div>
                        <p className="mt-0.5 sm:mt-2 text-xs sm:text-lg font-black text-foreground">{project.submissionCount}</p>
                    </div>
                    <div className="rounded-lg sm:rounded-xl border border-border bg-card p-1.5 sm:p-3 shadow-sm">
                        <div className="flex items-center gap-1 sm:gap-2 text-[7px] sm:text-xs font-bold uppercase tracking-wide text-muted-foreground">
                        ...
                        <p className="mt-0.5 sm:mt-2 text-[8px] sm:text-sm font-bold text-foreground truncate">{formatProjectDate(project.createdAt)}</p>
                    </div>
                    <div className="rounded-lg sm:rounded-xl border border-border bg-card p-1.5 sm:p-3 shadow-sm">
                        <div className="flex items-center gap-1 sm:gap-2 text-[7px] sm:text-xs font-bold uppercase tracking-wide text-muted-foreground">
                        ...
                        <p className="mt-0.5 sm:mt-2 text-[8px] sm:text-sm font-bold text-foreground line-clamp-1">
                ...
                <div className="flex items-center justify-between gap-2 border-t border-border pt-3 sm:pt-4">
                    <div className="min-w-0">
                        <p className="text-[7px] sm:text-[10px] uppercase tracking-wide text-muted-foreground">Auteur</p>
                        <p className="truncate text-[9px] sm:text-xs font-semibold text-foreground">{project.authorName}</p>
                    </div>
